...
/**
 * Handles the probabilities of a call happening in a given day, the duration of a call and the cost
 * of the call.
 *
...
public class DateTimeDistribution {
  ...
  public DateTimeDistribution(Configuration config) {
    outgoingCallParams = config.getOutgoingCallParams();

    loadOffPeakTimePeriod(config.getOffPeakTimePeriod());
    loadDayDist((Map<String, Double>) config.getDayDistribution());

    dateFormatter = DateTimeFormat.forPattern("dd/MM/yyyy");

    startDate = dateFormatter.parseDateTime(config.getStartDate());
    endDate = dateFormatter.parseDateTime(config.getEndDate());
    ...
    String timeDistFile = Optional.of(config).map(p -> p.getTimeDistCsv()).orElse(TIME_DIST_CSV);
  ...
  public void loadTimeDist(String filename) {
    ...
    try {
      ...
      if (JavaUtils.isJar() && filename.equals(TIME_DIST_CSV)) {
        ...
        if (filename.equals(TIME_DIST_CSV)) filename = "src/main/resources" + filename;

        listReader =
            new CsvListReader(
                new FileReader(filename), CsvPreference.EXCEL_NORTH_EUROPE_PREFERENCE);
      }
  ...
  public void loadOffPeakTimePeriod(Map<String, String> params) {
    ...
    offPeakStart =
        new LocalTime(Integer.parseInt(offPeakTime[0]), Integer.parseInt(offPeakTime[1]));
    ...
    while (tmpRnd > 0) {
      ...
      for (Map.Entry<String, Double> e : timeDistribution.get(type).entrySet()) {
        if (tmpRnd - e.getValue() > 0) {
          ...
          date =
              date.plusHours(Integer.parseInt(timeStr[0]))
   ...
   *
   * @param dayName A number between 1 and 7, corresponding to the day of the week, starting with
   *     Sunday.
  ...
  public int getCallDuration(
      ...
      String callType,
      LocalTime callTime,
      long avgCallDuration,
      long avgOPCallDuration) {
    String stdDevParam;
    long average = avgCallDuration;
    Map<String, Double> params = outgoingCallParams.get(callType);

    // Weekend
    // or within offPeak time period
    if ((dayName == 1 || dayName == 7)
        || (callTime.compareTo(offPeakStart) >= 0 && callTime.compareTo(offPeakEnd) <= 0)) {
    ...
    return Math.abs(
        RandomGaussian.generate(params.get(stdDevParam).longValue(), average)
            .getValueOne()
            .intValue());
  }
  ...
  /**
   * Get the cost for the call based on the type, duration and time period of the call.
   *
   * @param call The call to have its cost calculated, it remains unchanged.
   * @return The cost of the call
   */
  public double getCallCost(Interval time, String type) {
    int dayOfWeek = time.getStart().getDayOfWeek();
    int duration = time.toDuration().toStandardMinutes().getMinutes();
    LocalTime startTime = time.getStart().toLocalTime();

    Map<String, Double> params = outgoingCallParams.get(type);
    String callCostParam;

    if ((dayOfWeek == 1 || dayOfWeek == 7)
        || (startTime.compareTo(offPeakStart) >= 0 && startTime.compareTo(offPeakEnd) <= 0)) {
      callCostParam = "callOPCost";
    } else {
      callCostParam = "callCost";
    }

    long cost = params.get(callCostParam).longValue();
