...
public class PhoneBucketGenerator {
  private Map<String, Map<String, Long>> outgoingNumberDist;

  public PhoneBucketGenerator(Configuration config) {
    outgoingNumberDist = config.getOutgoingNumberDistribution();
  }
   ...
   */
  public Map<String, List<String>> createPhoneBucket(
      Person p, Map<String, Integer> callTypeSummary) {
    Map<String, Long> params;
    Map<String, List<String>> destPhoneNumbers =
        new HashMap<String, List<String>>(callTypeSummary.size());

    for (Map.Entry<String, Integer> e : callTypeSummary.entrySet()) {
      params = outgoingNumberDist.get(e.getKey());

      RandomGaussian gauss = RandomGaussian.generate(params.get("stdDev"), params.get("mean"));
