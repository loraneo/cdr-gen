...
/**
 * This class creates the defined number of customers and a set of calls following the gaussian
 * distribution.
 *
...
public class Population {
  ...
  public Population(Configuration config) {
    this.size = config.getNumAccounts();

    callsMade = config.getCallsMade();

    phoneLines = config.getPhoneLines();
    callTypes = config.getCallTypes();
  ...
  protected RandomGaussian getAvgCallDuration(String callType, boolean offPeak) {
    Map<String, Double> conf = outgoingCallParams.get(callType);

    String meanKey = (offPeak) ? "callOPDur" : "callDur";
    String stdDevKey = (offPeak) ? "callOPStdDev" : "callStdDev";

    return getRandomGaussian(conf.get(stdDevKey).longValue(), conf.get(meanKey).longValue());
  }
  ...
  /**
   * Create all calls for a given person according to the number of calls calculated for the person.
   *
  ...
  protected void createCalls(Person p) {
    ...
    for (int i = 0; i < p.getNumCalls(); i++) {
      String type = getType(listOfCallTypes, i);
      long avgCallDuration = p.getAvgCallDuration().get(type);
      long avgOPCallDuration = p.getAvgOffPeakCallDuration().get(type);

      Interval time = getTime(usedTimes, type, avgCallDuration, avgOPCallDuration);
      Call call =
          Call.builder()
              .id(i)
              .type(type)
              ...
              .destPhoneNumber(
                  phoneBucket
                      ...
                      .get(
                          RandomUtil.randInt(
                              0, phoneBucket.get(getType(listOfCallTypes, i)).size() - 1)))
              .time(time)
              .cost(dateTimeDist.getCallCost(time, type))
              .build();
  ...
  private String getType(String[] listOfCallTypes, int i) {
    return listOfCallTypes[i];
  }

  private Interval getTime(
      Map<String, List<Interval>> usedTimes,
      String callType,
    ...
    do {
      ...
      int duration =
          dateTimeDist.getCallDuration(
              currDayName, callType, dateTime.toLocalTime(), avgCallDuration, avgOPCallDuration);

      time = new Interval(dateTime, dateTime.plusMinutes(duration));
    } while (callIntervalOverlap(usedTimes, time));
    return time;
  }

  /**
   * Checks if a call time interval is already in use. This function is used to prevent to calls
   * from the same person happening at the same time.
   *
  ...
  protected boolean callIntervalOverlap(Map<String, List<Interval>> usedTimes, Interval time) {
    ...
    if (usedTimes.containsKey(date)) {
      for (Interval i : usedTimes.get(date)) {
        if (time.overlap(i) != null) return true;
      }
    } else {
