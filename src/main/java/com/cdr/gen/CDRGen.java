...
public final class CDRGen {
	...
	public static void saveToFile(List<Call> calls, Producer<String, String> producer) {
		...
		for (Call c : calls) {
			String message = new StringBuilder().append(c.getTime()).append(",").append(c.getCallerId()).append(",")
					.append(c.getFrom()).append(",").append(c.getTo()).append(",").append(c.getDisposition())
					.append(",").append(c.getSeconds()).append(",").append(c.getCost()).toString();
	...
	static List<Call> generateCalls(LocalDateTime now) {
		...
		for (int i = 0; i < numberOfCalls; i++) {
			...
			clls.add(Call.builder().time(now.format(java.time.format.DateTimeFormatter.ISO_DATE_TIME))
					.callerId(Integer.toString(i))
					.from(sourceNumber.getCountryCode() + "" + sourceNumber.getNationalNumber())
					.to(targetNumber.getCountryCode() + "" + targetNumber.getNationalNumber()).disposition("answered")
					.seconds(new Random().nextInt(120)).cost(0.02d).build());
		}
